...
interface DEXLibrary {
    ...
    function VelodromeGetAmountOut(
        address tokenFrom,
        address pool,
        uint256 inputAmount
    ) external view returns (uint256);
...
contract SmartAnalytics {
    ...
    address private constant VelodromeRouter = 0x9c12939390052919aF3155f41Bf4160Fd3666A6f;

    address private constant DEXLibraryAddress = 0x4C24B4C05a6d3c95AA911CE8577AD4B57aD1ed71;
    ...
    function findBestRouter(address tokenFrom, uint inputAmount, bytes memory encodedParams) internal view returns (uint bestAmountOut, address bestRouter, address bestPool, uint[] memory amountOutList) {
        ...
        for (uint i = 0; i < routers.length; i++) {
            ...
            else if (routers[i] == VelodromeRouter) {
                amountOut = DEXLibrary(DEXLibraryAddress).VelodromeGetAmountOut(tokenFrom, pools[i], inputAmount);
            }
