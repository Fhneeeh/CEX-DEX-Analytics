...
interface DEXLibrary {
    ...
    function EqualizerGetAmountOut(
        address tokenFrom,
        address pool,
        address router,
        uint256 inputAmount
...
contract SmartAnalytics {
    ...
    address private constant EqualizerRouter = 0x1A05EB736873485655F29a37DEf8a0AA87F5a447;

    address private constant DEXLibraryAddress = 0x86a9f70d451f353F3F1cF8abfceCcEC1391cD128;
    ...
    function findBestRouter(address tokenFrom, uint inputAmount, bytes memory encodedParams) internal view returns (uint bestAmountOut, address bestRouter, address bestPool, uint[] memory amountOutList) {
        ...
        for (uint i = 0; i < routers.length; i++) {
            ...
            if (routers[i] == EqualizerRouter) {
                amountOut = DEXLibrary(DEXLibraryAddress).EqualizerGetAmountOut(tokenFrom, pools[i], routers[i], inputAmount);
            }
